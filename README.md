...
| exclude  | []string | 排除的文件/目录正则表达式列表(文件夹名称匹配排除) | []     |
| event    | []string | 监听事件类型，c=创建 w=写入 m=修改 d=删除         | "cwmd" |
...
| 字段名 | 类型   | 说明                                                | 默认值 |
| ------ | ------ | --------------------------------------------------- | ------ |
| type   | string | 任务类型，echo, shell, service, oneshot, svg_sprite | ""     |
...
| ------ | -------- | ------------------------------------------------ |
| task   | string   | 触发的任务名称                                   |
| match  | []string | 匹配的文件路径正则表达式列表(文件全路径匹配触发) |
...
| env     | map      | 环境变量设置       | {}                 |
| dir     | string   | 执行命令的工作目录 | 配置文件所在目录   |
| timeout | duration | 命令执行超时时间   | 10s (service 忽略) |
...
tasks:
  ...
  build:
    type: oneshot
    command: ["go", "build", "-trimpath", "-ldflags", "-s -w", "-v"]
    env: { "GOOS": "linux" }
    dir: ./
    timeout: 30s
```
...
tasks:
  ...
  build:
    type: shell
    command: ['go build -trimpath -ldflags "-s -w" -o ./bin/app']
    timeout: 30s
...
2. 为每个任务指定唯一的名称，避免冲突
3. 为每个触发器指定唯一的名称，避免冲突
4. 长任务（如服务进程）建议使用 `service` 类型，而不是 `oneshot`，以避免超时问题
5. shell 任务每一条命令, 逐条执行
6. 合理设置 `throttle` 参数，避免频繁触发任务
7. 使用 `exclude` 参数排除不必要的文件和目录，提高性能

## 许可证

fsw 基于 MIT 许可证开源，您可以在遵守许可证条款的前提下自由使用、修改和分发该软件。
