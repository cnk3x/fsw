...
type Config struct {
	...
	Root     []string         `json:"root"`     // 监听主目录列表，支持多个目录
	Event    string           `json:"event"`    // 监听事件类型，如 c=创建 w=写入 m=修改 d=删除
	Exclude  []string         `json:"exclude"`  // 排除匹配规则，符合这些规则的文件将被忽略
	Throttle configx.Duration `json:"throttle"` // 节流时间，防止短时间内重复触发

	Tasks    map[string]*Typed `json:"tasks"`    // 任务配置映射，键为任务名称
	Triggers []Trigger         `json:"triggers"` // 触发器列表，定义文件变化时执行的任务
}

// Trigger 定义单个触发器规则
type Trigger struct {
	Match []string `json:"match"` // 匹配文件路径的正则表达式列表，匹配成功则触发
